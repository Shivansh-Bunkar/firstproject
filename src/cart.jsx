...
function Cart() {
    ...
    const gstAmount = priceAfterDisc * gst;
    ...
    let orderId = Math.floor(Math.random() * 1000000); // Generate a random order ID for demonstration

    let shippingCost = 50; // Flat shipping cost for demonstration  
    let templateParams = {
        ...
        email: email       // customer's email, required for {{email}}
    ...
    let handleCheckoutEmail = () => {
        emailjs.send("service_170bn2j", "template_agchm6g", templateParams, "QIhxov6-5_kkxM4di")
            .then((res) => toast.success("Checkout email sent successfully!", {
                duration: 3000,
                style: {
                    background: "#4CAF50",
                    color: "white"
            ...
            .catch((err) => {
                ...
                console.error("Email sending failed, error", err);
            });
            ...
            <div className="cart-page">
                {items.length === 0 ? (
                    ...
                    <>
                        ...
                        <button className="checkout" onClick={() => setCheckout(true)}>   Checkout   </button>
                        {checkout && (
                        ...
                        {paymentMethod === "qr" && (
                            <div className="payment">
                                ...
                                <p>Final Amount to Pay: ₹{finalAmount.toFixed(2)}</p>
                                <QRCode
                                    value={`upi://pay?pa=7389959273@nyes&pn=Shivansh-Bunkar&am=${Number(finalAmount).toFixed(2)}&cu=INR`}
                                />
                            </div>
                        )}

                        {
                            <div id="div-cont">
                                ...
                                <label> Enter email for confirmation message to your email: </label>
